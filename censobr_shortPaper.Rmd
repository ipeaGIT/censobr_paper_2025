...
The harmonization of the 1960 Census dataset presented in `{censobr}` stems from prior work by one of the authors, but until now, it had not been made publicly available. Thus, `{censobr}` represents the first time this dataset is accessible to the broader academic community, offering a unique and valuable resource for researchers interested in Brazil’s demographic history. The microdata version we now make available represents a combination of two distinct datasets drawn from the 1960 population census. 
